...
// 	Organization organization = OrganizationLocalServiceUtil.getOrganization(group.getClassPK());
	...
	<c:otherwise>
		<div class="lfr-message-info">
			...
			<%
			System.out.println("!!gyl this-application-will-only-function-when-placed-on-an-organization-page error");
			%>
		</div>
	</c:otherwise>
</c:choose> --%>
...
<liferay-util:buffer var="html">
	...
	<%-- <div id="<portlet:namespace />updateOrganizationDialog">
			<aui:form action="" method="post" name="dialogForm" onSubmit="<%= taglibOnSubmit %>">
			<aui:input name="redirect" type="hidden"  value="<%= organization.getDisplayURL(themeDisplay) %>" />
			<aui:input name="fieldGroup" type="hidden"  value="<%= curSectionId %>" />
...
// 			request.setAttribute("organization.organization", organization);

	/* 		String xxx=Organization.class.getName();
			System.out.println(xxx);
// 			request.setAttribute("addresses.className", Organization.class.getName());
			request.setAttribute("addresses.className", xxx);
			...
			long yyy = organization.getOrganizationId();
			System.out.println(yyy);
			if (organization != null) {
// 				request.setAttribute("addresses.classPK", organization.getOrganizationId());
				request.setAttribute("addresses.classPK", yyy);
...
// 			String sectionJsp = "/html/organizationprofile/" + _getSectionJsp(curSectionId) + ".jsp";
			%>
