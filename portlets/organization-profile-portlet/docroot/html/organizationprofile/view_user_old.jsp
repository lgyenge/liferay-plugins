...
if (userId > 0) {
	user2 = UserLocalServiceUtil.getUser(userId); // aki be van jelentkezve ???
}
else {
	user2 = (User)request.getAttribute(WebKeys.CONTACTS_USER); // akié a profile ???
}
			...
			<aui:layout cssClass="social-relations">
				...
				<aui:layout cssClass="contacts-action">
					<c:choose>
						...
						<c:otherwise>
							<liferay-util:include page="/html/organizationprofile/user_toolbar.jsp" servletContext="<%= application %>" />
						</c:otherwise>
					</c:choose>
				</aui:layout>
					...
					<c:if test="<%= showUsersInformation %>">
						<aui:column columnWidth="<%= showSites ? 80 : 100 %>" cssClass="user-information-column-1">
							...
							<div class="lfr-user-info-container">
								<liferay-util:include page="/html/organizationprofile/view_user_information.jsp" servletContext="<%= application %>" />
							</div>
...
<c:if test="<%= themeDisplay.getUserId() == user2.getUserId() %>">
	<aui:script use="aui-base">
			...
			var <portlet:namespace />openDialog = function(event) {
				...
				var uri = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"><portlet:param name="mvcPath" value="/html/organizationprofile/edit_user_dialogs.jsp" /></portlet:renderURL>';
